...
namespace Northwind.DataAccess.SqlServer.SqlDao
{
    ...
    /// </summary>
    public sealed class ProductCategorySqlServerDao : IProductCategoryDAO
    {
        private readonly SqlConnection connection;

        /// <summary>
        /// Initializes a new instance of the <see cref="ProductCategorySqlServerDao"/> class.
        /// </summary>
        /// <param name="connection">A <see cref="SqlConnection"/>.</param>
        public ProductCategorySqlServerDao(SqlConnection connection)
        {
...
FETCH FIRST {1} ROWS ONLY";
        ...
        /// <inheritdoc/>
        public IAsyncEnumerable<ProductCategoryTransferObject> SelectProductCategoriesByNameAsync(ICollection<string> productCategoryNames)
        {
...
SELECT @@ROWCOUNT";
        ...
        private async IAsyncEnumerable<ProductCategoryTransferObject> ExecuteReader(string commandText)
        {
            await using var command = new SqlCommand(commandText, this.connection);
            await this.connection.OpenAsync();
            await using var reader = await command.ExecuteReaderAsync();
